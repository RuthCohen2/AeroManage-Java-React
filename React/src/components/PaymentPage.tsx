...
//     // עדכון: שליפת הטיסה הספציפית ישירות מהנתיב היעיר ב-Java
//     useEffect(() => {
//         fetch(`http://localhost:8081/api/flights/${id}`)
//             .then(res => {
//                 if (!res.ok) throw new Error("לא הצלחנו למצוא את פרטי הטיסה המבוקשת");
//                 return res.json();
//             })
...
//     // עדכון הפונקציה: מבצעת קריאה לשרת ומפחיתה מקום לפני אישור ההזמנה
...
//         // התחלת תהליך העדכון ב-Java
//         setLoading(true);
//         fetch(`http://localhost:8081/api/flights/${id}/book`, {
//             method: 'PUT',
//             headers: {
...
//         .then(res => {
//             if (!res.ok) {
//                 throw new Error('אזלו המקומות בטיסה זו, או שההזמנה נכשלה בשרת');
//             }
//             return res.json();
//         })
//         .then((updatedFlight) => {
//             console.log("המקום עודכן בהצלחה בבסיס הנתונים:", updatedFlight);
//             setLoading(false);
//             setIsSubmitted(true); // מעביר למסך ההצלחה המעוצב שלך
//         })
//         .catch(err => {
//             setBackendError(err.message);
//             setLoading(false);
...
//     if (loading) {
//         return <div className="payment-container"><div className="loading-text">⏳ מעבד נתונים ומאבטח מקום בטיסה...</div></div>;
//     }
...
//                     <p className="success-subtitle">איזה כיף, חוויית הטיסה שלך מתחילה כאן.</p>
                    
//                     <div className="ticket-summary">
//                         <div className="ticket-row">
//                             <span>יעד הטיסה:</span>
//                             <strong>{flight ? flight.destination : 'לא ידוע'}</strong>
//                         </div>
//                         <div className="ticket-row">
//                             <span>נקודת מוצא:</span>
//                             <span>{flight ? flight.departureFrom : 'לא ידוע'}</span>
//                         </div>
//                         <div className="ticket-row">
//                             <span>חברת תעופה:</span>
//                             <span>{flight ? flight.airline : 'לא ידוע'} ({flight?.flightNumber})</span>
//                         </div>
//                         <div className="ticket-row">
//                             <span>שם הנוסע:</span>
...
//                     <p className="email-notice">כרטיסי הטיסה האלקטרוניים ואישור ההזמנה נשלחו לכתובת המייל שלך.</p>
...
//                 {backendError && <div style={{ color: '#dc3545', marginBottom: '15px', fontWeight: 'bold' }}>❌ שגיאה: {backendError}</div>}
