...
// import { Link, useNavigate } from 'react-router-dom'; // 1. ייבוא useNavigate
...
//     const navigate = useNavigate(); // 2. הגדרת הניווט

//     useEffect(() => {
//         // שליפת הטיסות משרת ה-Java (פורט 8081)
...
//                 {flights.map((flight) => (
//                     /* עטיפת כל כרטיס הטיסה ב-Link כדי להפוך אותו לקישור לעמוד הספציפי שחברה שלך בונה */
//                     <Link to={`/flights/${flight.id}`} key={flight.id} className="flight-card-link">
...
//                                 <span className="from">{flight.departureFrom}</span> 
//                                 <span className="arrow">➔</span> 
//                                 <span className="to">{flight.destination}</span> 
//                             </div>
                            
//                             <div className="flight-details"> 
//                                 <p><strong>תאריך:</strong> {new Date(flight.departureTime).toLocaleString('he-IL')}</p> 
//                                 <p><strong>מקומות נותרים:</strong> {flight.availableSeats}</p> 
//                             </div>
...
//                                     onClick={(e) => {
//                                         // מונע מהלחיצה על הכפתור להפעיל גם את הניווט של הקישור שעוטף את הכרטיס
//                                         e.stopPropagation(); 
//                                         e.preventDefault();
                                        
//                                         // 3. מעבר ישיר לדף התוקף והתשלום עם ה-ID של הטיסה הזו
//                                         navigate(`/checkout/${flight.id}`);
//                                     }}
//                                 >
//                                     {flight.availableSeats > 0 ? 'הזמן כרטיס' : 'אזל המלאי'} 
//                                 </button>
//                             </div>
...
//     // שני משתני הסטייט החדשים עבור הסינון המשולב שלנו
//     const [searchDestination, setSearchDestination] = useState<string>('');
//     const [maxPrice, setMaxPrice] = useState<number>(1000); // ברירת מחדל תקציב של 1000
    
//     const navigate = useNavigate();

//     // פונקציה שמבצעת את קריאת ה-fetch לשרת
//     const fetchFilteredFlights = () => {
//         setLoading(true);
//         // פנייה ל-Endpoint החדש ב-Java עם ה-Query Parameters של הסינון
...
//                             position: 'relative'
//                         }}>
//                             {/* תגית ביקוש גבוה במידה והתמחור דינמי (פחות מ-10 מקומות) */}
...
//                             <p style={{ margin: '5px 0', fontSize: '14px', color: '#b3b3b3' }}>חברה: {flight.airline}</p>
//                             <p style={{ margin: '5px 0', fontSize: '14px', color: '#b3b3b3' }}>מאיפה: {flight.departureFrom}</p>
//                             <p style={{ margin: '5px 0', fontSize: '14px', color: '#b3b3b3' }}>מקומות פנויים: <span style={{ color: flight.availableSeats <= 10 ? '#ff6b6b' : '#28c76f' }}>{flight.availableSeats}</span></p>
...
//                                     style={{ padding: '6px 12px', backgroundColor: '#28c76f', color: '#fff', border: 'none', borderRadius: '4px', cursor: 'pointer' }}
//                                 >
//                                     הזמן כעת
//                                 </button>
//                             </div>
